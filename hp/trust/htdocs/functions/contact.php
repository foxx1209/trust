...
// $site_top = 'http://www.skyjapan-corp.com/';

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
// common.phpで管理
$to = $to_contact;
// $to = "main.plusinc@gmail.com,yuichiro.odachi@gmail.com";
// $to = "information@skyjapan-corp.co.jp";

//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
$from = "information@skyjapan-corp.co.jp";
...
$require = array('名前','フリガナ','メールアドレス','お問い合わせ内容','個人情報の取り扱い規定');
...
〒541-0042 大阪市中央区今橋4-1-1 淀屋橋三井ビルディング4階
tel.06-6209-8260(代表)
fax.06-6209-8262
──────────────────────
...
$btn_send='送信';
$btn_ret='戻る';
$msg_error='入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。';
$msg_hissu='は必須項目です。';
$btn_cap_back='前画面に戻る';
$title_comp='完了画面';
if($_GET['lang']=='en') {
	...
	$btn_send='Send';
	$btn_ret='Back';
	$msg_error='There is an error in your input. Please check the following and make corrections by clicking the "Back" button.';
	$msg_hissu=' is a required field.';
	$btn_cap_back='Return to previous screen';
	$title_comp='Completion screen';
	$require = array('name','mailaddress','お問い合わせ内容','個人情報の取り扱い規定');
}
...
// echo '$confirmDsp → '.$confirmDsp.'<br>';
// echo '$empty_flag → '.$empty_flag.'<br>';
...
<link rel="stylesheet" href="<?php echo $parent.'css/'.$contact['link'].'.css?'.$version; ?>">
...
.p-contact .__body .__question {
	text-align: center;
	font-size: 17px;
	margin-bottom: 1.5em;
}
.p-contact .__body .__send {
	
}
.p-contact .__body .__send > p {
  display: flex;
...
.p-contact .__body .__send input {
	font-size: 13px;
	color: #fff;
	padding: 1em 3em;
}
.p-contact .__body .__send input:nth-of-type(3) {
	background-color: #CCCCCC;
}
.p-contact .__body .__send input:nth-of-type(4) {
	background-color: #006591;
}

.p-contact .__body .__error {
	text-align: center;
}
.p-contact .__body .__error h4 {
	font-size: 17px;
	margin-bottom: 2em;
}
.p-contact .__body .__error > input {
	font-size: 13px;
	color: #fff;
	padding: 1em 3em;
	margin: 0 auto;
...
<!-- ▼************ 送信内容表示部　※編集は自己責任で ************ ▼-->
<main id="<?php echo $contact['link'] ?>" class="p-contact">
	<div class="container__inner">

		<div class="__head">
			<h2 class="c-ttl__subpage__02 en"><?php echo $contact['title_jp'] ?></h2>
			<p class="c-description__31"><?php echo $contact['description'] ?></p>
		</div>
		...
		<div class="__body">
			...
			<div id="formWrap">
				...
				<div class="__error">
					...
					<?php echo $errm; ?>
					<br><br>
					<input type="button" value=" <?php echo $btn_cap_back; ?> " onClick="history.back()">
				</div>

				<?php }else{ ?>

				<form action="<?php echo h($_SERVER['SCRIPT_NAME']).'?lang='.$_GET['lang']; ?>" method="POST">
					...
					<dl class="p-form">
						...
						<dd class="c-lang-wrap">
							<span class="__jp"><?php echo $_POST['名前']; ?></span>
							<span class="__en"><?php echo $_POST['name']; ?></span>
						</dd>
						<dt class="c-lang-wrap">
						...
						<dd class="c-lang-wrap">
							<span class="__jp"><?php echo $_POST['フリガナ']; ?></span>
							<span class="__en"><?php echo $_POST['noneed']; ?></span>
						</dd>
						<dt class="c-lang-wrap">
						...
						<dd class="c-lang-wrap">
							<span class="__jp"><?php echo $_POST['メールアドレス']; ?></span>
							<span class="__en"><?php echo $_POST['mailaddress']; ?></span>
						</dd>
						<dt class="c-lang-wrap">
							<span class="__hissu __jp">お問い合わせ内容</span>
							<span class="__hissu __en">Inquiry detail</span>
						</dt>
						<dd><?php echo $_POST['お問い合わせ内容']; ?></dd>
					</dl>
					...
					<p class="__question c-lang-wrap">
						<span class="__jp">入力された内容で送信しますか？</span>
						<span class="__en">Do you want to send with what you typed?</span>
					</p>
					<div class="__send">
						<p align="center">
							<input type="hidden" name="mail_set" value="confirm_submit">
							<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
							<input type="button" value="<?php echo $btn_ret; ?>" onClick="history.back();">
							<input type="submit" value="<?php echo $btn_send; ?>">
						</p>
					</div>
...
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" />
<meta name="format-detection" content="telephone=no">
<title><?php echo $title_comp; ?></title>
</head>
<body>
<div align="center">
<?php if($empty_flag == 1){ ?>
...
<br /><br /><input type="button" value=" <?php echo $btn_cap_back; ?> " onClick="history.back()">
</div>
</body>
</html>
<?php }else{ ?>
送信ありがとうございました。<br />
送信は正常に完了しました。<br /><br />
<a href="<?php echo $site_top.'?lang='.$_GET['lang'] ;?>">トップページへ戻る&raquo;</a>
</div>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4><?php echo $msg_error; ?></h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" <?php echo $btn_cap_back; ?> " onClick="history.back()"></div>
<?php 
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".h($key)."】".$msg_hissu."</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
...
function copyright(){
	// echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="http://www.mikami-marine.co.jp/" target="_blank">- 三上船舶工業株式会社 -</a>';
}
//----------------------------------------------------------------------
//  関数定義(END)
//----------------------------------------------------------------------
